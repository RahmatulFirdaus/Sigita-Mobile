...
class GetSigita {
  ...
  static Future<List<GetSigita>> connApi() async {
    Uri url = Uri.parse("http://10.0.10.61:3000/api/getPostingan");
  ...
  static Future<GetSigita> connApiDetail(String id) async {
    Uri url = Uri.parse("http://10.0.10.61:3000/api/getPostinganDetail/$id");
    var hasilResponse = await http.get(url);
    var jsonData = jsonDecode(hasilResponse.body);
...
class PostSigita {
  ...
  static Future<PostSigita> postSigita(
    ...
    Uri url = Uri.parse("http://10.0.10.61:3000/api/simpanKomentar");
...
class GetFile {
  ...
  static Future<GetFile> getFile(String id) async {
    Uri url = Uri.parse("http://10.0.10.61:3000/api/getDownloadFile/$id");
    var hasilResponse = await http.get(url);
    var jsonData = jsonDecode(hasilResponse.body);
...
class PermissionFile {
  ...
  static Future<PermissionFile> postDownload(
    ...
    Uri url = Uri.parse("http://10.0.10.61:3000/api/downloadModul");
    var hasilResponse = await http.post(
      url,
      body: {
        "id_postingan": idPostingan,
...
class GetKategori {
  ...
  static Future<List<GetKategori>> getKategori() async {
    Uri url = Uri.parse("http://10.0.10.61:3000/api/getKategori");
    var hasilResponse = await http.get(url);
    var jsonData = jsonDecode(hasilResponse.body);
    var dataList = jsonData["data"] as List;
    return dataList.map((user) {
...
class GetKomentar {
  ...
  static Future<List<GetKomentar>> getKomentar(String id) async {
    Uri url = Uri.parse("http://10.0.10.61:3000/api/getKomentar/$id");
    var hasilResponse = await http.get(url);
    var jsonData = jsonDecode(hasilResponse.body);
    var dataList = jsonData["data"] as List;
    return dataList.map((user) {
